package com.doggie.RhoduimMod.blocks.recipes;
...
private BlastFurnaceRecipes()
{
    ...
    this.addSmeltingRecipeForBlock(ModBlocks.RHODUIM_ORE, new ItemStack(ModItems.RHODUIM_INGOT), 1.0F);
...
{
    if (getBlastResult(input) != ItemStack.EMPTY) { net.minecraftforge.fml.common.FMLLog.log.info("Ignored smelting recipe with conflicting input: {} = {}", input, stack); return; }
 ...
 * @param itemstack 
 */
public ItemStack getBlastResult( ItemStack itemstack)
{
 ...
 */
private boolean compareItemStacks(ItemStack input, ItemStack input2)
{
    return input2.getItem() == input.getItem() && (input2.getMetadata() == 32767 || input2.getMetadata() == input.getMetadata());
}
